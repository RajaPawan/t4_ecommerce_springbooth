...
public class ConsumerController {
	...
	public ResponseEntity<Object> putCart(@RequestBody CartProduct cp) {
		...
		Optional<CartProduct> cp1 = cartProductRepo.findByCartUserUserIdAndProductProductId
                (userRepo.findByUsername(jwtUtil.getCurrentUser()).get().getUserId() , cp.getProduct().getProductId());
		if(cp1.isEmpty()) {
			if(cp.getQuantity()>0) {
	...
	public ResponseEntity<Object> deleteCart(@RequestBody Product p) {
		Optional<CartProduct> cp1 = cartProductRepo.findByCartUserUserIdAndProductProductId
                (userRepo.findByUsername(jwtUtil.getCurrentUser()).get().getUserId() , p.getProductId());
		if(cp1.isPresent()) {
